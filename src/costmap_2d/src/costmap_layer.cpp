...
void CostmapLayer::updateWithMax(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  ...
  unsigned char* master_h = master_grid.getHeightMap(); //031022 Kangneoung Lee heightmap
  unsigned char* master_ground = master_grid.getGroundCharMap(); //031022 Kangneoung Lee groundcostmap
  ...
  for (int j = min_j; j < max_j; j++)
  {
    ...
    for (int i = min_i; i < max_i; i++)
    {
      ...
      unsigned char old_height  = master_h[it];  //031022 Kangneoung Lee heightmap
      if (old_height < heightmap_[it])  //031022 Kangneoung Lee heightmap
        master_h[it] = heightmap_[it];
		
      unsigned char old_groundcost  = master_ground[it];  //031022 Kangneoung Lee groundcostmap
      if (old_groundcost < groundcostmap_[it])  //031022 Kangneoung Lee groundcostmap
        master_ground[it] = groundcostmap_[it];
      it++;
    }
  }
}
...
void CostmapLayer::updateWithOverwrite(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  ...
  for (int j = min_j; j < max_j; j++)
  {
    ...
    for (int i = min_i; i < max_i; i++)
    {
	  master_h[it] = heightmap_[it]; //031022 Kangneoung Lee heightmap
	  master_ground[it] = groundcostmap_[it]; //031022 Kangneoung Lee groundcostmap
      if (costmap_[it] != NO_INFORMATION)
        master[it] = costmap_[it];
      it++;
    }
